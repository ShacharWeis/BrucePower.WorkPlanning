...
namespace pointcloudviewer.binaryviewer
{
    public class PointCloudViewerTilesDX11 : MonoBehaviour
    {
        ...
        public int minimumTilePointCount = 0;
        ...
        void Start()
        {
            ...
            if (loadAtStart == true)
            {
                ...
                // index file for all tiles
                LoadRootFile(rootFile);
            }
        ...
        // TODO make enum
        internal bool isPackedColors = false;
        internal bool isIntPacked = false;
        internal bool isClassificationPacked = false;
        internal bool useLossyBytePacking = false;
        ...
        internal float gridSizePackMagic = 0;

        public bool overridePosition = false;
        public Vector3 offsetPosition = Vector3.zero;
        int largestTilePointCount = 0;
        ...
        void LoadRootFile(string filePath)
        {
            initIsReady = false;
            ...
            Thread loadThread = new Thread(() =>
            {
                ...
                for (int tileRow = rowCountIndex; tileRow < rootData.Length; tileRow++)
                {
                    ...
                    {
                        filenamesRGB[items] = Path.Combine(rootFolder, row[0]);
                    }
                ...
                MainThread.Call(OnRootLoadingComplete);
            });

            loadThread.Start();
        } // LoadRootFile

        void OnRootLoadingComplete()
        {
            for (int i = 0; i < tilesCount; i++)
            {
                ...
                if (useMeshRendering == true)
                {
                    ...
                    mf.name = "MeshTile_" + i;// + "_x=" + t.cellX + "_y=" + t.cellY + "_z=" + t.cellZ;
        ...
        } // OnRootLoadingComplete()

        IEnumerator ProfileLoaderStatus()
        {
        ...
        IEnumerator InitDX11Buffer(object index)
        {
            ...
            {
                // lazily create material (instead of during init, which can be slow for +100k tiles)
                if (tiles[nodeIndex].material == null)
                {
                    tiles[nodeIndex].material = new Material(cloudMaterial);

                    // set offset for packed
                    if (isPackedColors == true)
                    {
                        tiles[nodeIndex].material.SetVector("_Offset", new Vector3(tiles[nodeIndex].cellX * gridSize, tiles[nodeIndex].cellY * gridSize, tiles[nodeIndex].cellZ * gridSize));
                        gridSizePackMagic = gridSize * packMagic;
                        tiles[nodeIndex].material.SetFloat("_GridSizeAndPackMagic", gridSizePackMagic);
                    }
                    ...
                    if (overridePosition == true)
                    {
                        tiles[nodeIndex].material.SetVector("_Offset", offsetPosition);
                    }
                }
