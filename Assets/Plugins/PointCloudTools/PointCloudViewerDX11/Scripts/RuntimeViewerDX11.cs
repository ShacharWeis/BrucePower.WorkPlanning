...
namespace PointCloudRuntimeViewer
{
    ...
    public class RuntimeViewerDX11 : MonoBehaviour
    {
        ...
        public void LoadRawPointCloud(System.Object a)
        {
            ...
            // OPT: Use BufferedStream with large buffer for faster I/O
            const int ReadBufferSize = 1024 * 1024; // 1 MB buffer

            // get initial data (so can check if data is ok)
            using (var fileStream = new FileStream(fullPath, FileMode.Open, FileAccess.Read, FileShare.Read, ReadBufferSize, FileOptions.SequentialScan))
            using (StreamReader streamReader = new StreamReader(new BufferedStream(fileStream, ReadBufferSize)))
            {
                double x = 0, y = 0, z = 0;
                ...
                // OPT: Pre-cache separator array to avoid per-call allocation
                ...
                { // other formats need to be read completely
                    ...
                    // OPT: Estimate line count from file size to avoid counting pass when possible
                    // Rough estimate: average ~30 bytes per line for XYZ data
                    long remainingBytes = streamReader.BaseStream.Length - streamReader.BaseStream.Position;
                    long estimatedLines = remainingBytes / 25; // conservative estimate
                ...
                // OPT: Cache booleans to local variables for tight loop
                bool doReadRGB = readRGB;
                bool doReadIntensity = readIntensity;
                bool doUseUnitScale = useUnitScale;
                bool doAutoOffset = autoOffsetNearZero;
                bool doUseManualOffset = useManualOffset;
                bool doFlipYZ = flipYZ;
                float localUnitScale = unitScale;
                float offsetX = manualOffset.x;
                float offsetY = manualOffset.y;
                float offsetZ = manualOffset.z;
                bool doHasNormals = plyHasNormals;
                bool doHasDensity = plyHasDensity;
                PointCloudFormat localFormat = pointCloudFormat;

                // OPT: Reusable char buffer for fast inline parsing
                // (avoids string.Replace + string.Split allocations)
                ...
                while (haveMoreToRead == true && abortReaderThread == false)
                {
                    ...
                    {
                        // OPT: Use StringSplitOptions.RemoveEmptyEntries instead of Replace chains
                        row = line.Split(separators, StringSplitOptions.RemoveEmptyEntries);
                        ...
                        if (row.Length > 2)
                        {
                            switch (localFormat)
                            {
                                case PointCloudFormat.ASC: // ASC
                                    ...
                                    // OPT: Use TryParse with out var for faster failure path
                                    if (!double.TryParse(row[0], NumberStyles.Float, CultureInfo.InvariantCulture, out x)) { skippedRows++; continue; }
                                    if (!double.TryParse(row[1], NumberStyles.Float, CultureInfo.InvariantCulture, out y)) { skippedRows++; continue; }
                                    if (!double.TryParse(row[2], NumberStyles.Float, CultureInfo.InvariantCulture, out z)) { skippedRows++; continue; }
                                    break;
                                ...
                                case PointCloudFormat.CGO: // CGO	(counter at first line and uses comma)
                                    ...
                                    // OPT: Use Replace only on individual tokens, not the whole line
                                    if (!double.TryParse(row[0].Replace(',', '.'), NumberStyles.Float, CultureInfo.InvariantCulture, out x)) { skippedRows++; continue; }
                                    if (!double.TryParse(row[1].Replace(',', '.'), NumberStyles.Float, CultureInfo.InvariantCulture, out y)) { skippedRows++; continue; }
                                    if (!double.TryParse(row[2].Replace(',', '.'), NumberStyles.Float, CultureInfo.InvariantCulture, out z)) { skippedRows++; continue; }
                                    break;
                                ...
                                case PointCloudFormat.CATIA_ASC: // CATIA ASC (with header and Point Format           = 'X %f Y %f Z %f')
                                    ...
                                    if (!double.TryParse(row[1], NumberStyles.Float, CultureInfo.InvariantCulture, out x)) { skippedRows++; continue; }
                                    if (!double.TryParse(row[3], NumberStyles.Float, CultureInfo.InvariantCulture, out y)) { skippedRows++; continue; }
                                    if (!double.TryParse(row[5], NumberStyles.Float, CultureInfo.InvariantCulture, out z)) { skippedRows++; continue; }
                                    break;
                                ...
                                case PointCloudFormat.PLY_ASCII: // PLY (ASCII)
                                    ...
                                    if (doReadRGB == true)
                                    {
                                        ...
                                        else if (doHasDensity == false) // no normals or density
                                        {
                                            r = LUT255[int.Parse(row[3], CultureInfo.InvariantCulture)];
                                            g = LUT255[int.Parse(row[4], CultureInfo.InvariantCulture)];
                                            b = LUT255[int.Parse(row[5], CultureInfo.InvariantCulture)];
                                ...
                                default:
                                    Debug.LogError("> Error Unknown format:" + localFormat);
                                    break;

                            } // switch

                            // OPT: Inline scaling + offset math to reduce branching in tight loop
                            if (doUseUnitScale == true)
                            {
                                x *= localUnitScale;
                                y *= localUnitScale;
                                z *= localUnitScale;
                            ...
                            if (doAutoOffset == true || doUseManualOffset == true) // NOTE: can use only one at a time
                            {
                                x -= offsetX;
                                y -= offsetY;
                                z -= offsetZ;
                            ...
                            // if have color data
                            if (doReadRGB == true || doReadIntensity == true)
                            {
            ...
            if (cacheBinFile == true)
            {
                ...
                // OPT: Use BufferedStream for binary write
                using (var bsOut = new BufferedStream(new FileStream(outputFile, FileMode.Create, FileAccess.Write, FileShare.None, ReadBufferSize), ReadBufferSize))
                using (var writer = new BinaryWriter(bsOut))
                {
            ...
            stopwatch.Stop();
            Debug.LogFormat("Timer: {0} s", stopwatch.Elapsed);
            stopwatch.Reset();
